      ...
      <div className="flex-1 lg:pl-72 flex flex-col">
        ...
        <main className="flex-1 p-10 space-y-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <h1 className="text-4xl font-black italic tracking-tighter text-foreground uppercase">Assignment Vault</h1>
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/40 mt-2">Managing Pedagogical Deliverables</p>
            </div>
            <button 
              onClick={() => {
          ...
          <div className="bg-card rounded-[3rem] shadow-premium border border-border overflow-hidden">
            <div className="p-10 border-b border-border flex flex-col md:flex-row gap-6 md:items-center justify-between bg-background/30">
              ...
              <div className="flex gap-4">
                <div className="h-14 px-6 rounded-2xl bg-background/50 border border-border flex items-center gap-3 text-sm font-bold text-muted-foreground/40">
            ...
            <div className="p-10 bg-card">
              {loading ? (
                <div className="flex flex-col items-center justify-center py-24 gap-4 opacity-50">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Syncing Vault...</p>
                </div>
              ) : filteredAssignments.length === 0 ? (
                <div className="bg-background/20 p-20 rounded-[3rem] border border-dashed border-border flex flex-col items-center justify-center text-center">
                  ...
                  <h2 className="text-xl font-black italic uppercase tracking-tight text-foreground">Vault Depleted</h2>
                  <p className="text-sm text-muted-foreground/40 mt-2 max-w-xs">No active assignments matching your query.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                  {filteredAssignments.map((item) => (
                    <div key={item.id} className="p-10 bg-background/40 rounded-[2.5rem] border border-border shadow-sm flex flex-col justify-between group hover:shadow-xl transition-all duration-500 relative overflow-hidden">
                       {/* Department Indicator */}
                       <div className="absolute top-0 right-0 p-1 bg-primary/10 rounded-bl-[1.5rem] text-[9px] font-black uppercase tracking-[0.2em] px-4 text-primary italic">
                          {item.department}
                       </div>
                       ...
                       <div className="space-y-6">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                               <h3 className="text-lg font-black italic uppercase tracking-tighter text-foreground">{item.title}</h3>
                               <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Target Segment: {item.department}</p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between border-t border-border pt-6">
                             ...
                             <div className="flex items-center gap-3">
                                <button 
                                  ...
                                  className="h-10 w-10 flex items-center justify-center rounded-xl bg-white/5 text-muted-foreground/40 hover:text-primary hover:bg-primary/10 transition-all"
                                >
                                  <Edit3 className="h-5 w-5" />
                                </button>
                                <button 
                                  onClick={() => handleDelete(item.id!, item.title)}
                                  className="h-10 w-10 flex items-center justify-center rounded-xl bg-white/5 text-muted-foreground/40 hover:text-rose-500 hover:bg-rose-500/10 transition-all"
                                >
